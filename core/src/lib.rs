...
pub const APP_VERSION: &str = "0.2.9";
...
pub const VERSION_HISTORY: &[(&str, &str, &str)] = &[
    ...
    (
        "0.2.8",
        "2026-07-25",
        "소스 HTTP 호출에 엔드포인트별 타임아웃 추가 — ureq는 전체 타임아웃 기본값이 없어, 소스가 \
         연결만 받고 응답하지 않으면 무한 대기했다. sync_source가 store 뮤텍스를 쥔 채 도는 구조라 \
         소스 하나가 멈추면 앱 전체가 잠길 수 있었다. /health 5초 · /keywords 20초 · /vectors 90초 \
         (벡터는 소스가 키워드 N개를 그 자리에서 임베딩하므로 넉넉히 준다).",
    ),
    (
        "0.2.9",
        "2026-07-25",
        "SVIL 디자인 가이드 검수 — 기본 글꼴을 현행 표준인 라인시드로 교체하고(구 표준 교보손글씨2019 \
         기준으로 만들어져 있었음) 글꼴 목록 순서도 가이드와 맞췄다. 이미 글꼴을 고른 사용자의 설정은 \
         그대로 유지된다. 색상 토큰·글자 크기 3단계·최소 12px·Consolas 모노·버튼/포커스/탭 규격·\
         배지의 색+텍스트 라벨 병행은 검수 결과 이미 현행 가이드에 적합.",
    ),
];
