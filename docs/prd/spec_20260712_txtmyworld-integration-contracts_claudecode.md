...
### 6.4 TXTBrain·TXTAIMemory 전용 Keyword API는 토큰이 필요

허브와 달리 개별 소스에 직접 붙으려면(전략 A 세밀 제어, 또는 허브가 꺼져 있을 때) 각 앱 자신의 승인 화면에서 발급한 토큰이 필요하다(`/keywords`가 `401`류 오류 반환 확인). TXTMyWorld는 토큰을 대신 발급할 수 없다 — 사용자가 해당 앱에서 직접 받아와야 한다. 허브 경유가 훨씬 마찰이 적은 기본 경로다.
