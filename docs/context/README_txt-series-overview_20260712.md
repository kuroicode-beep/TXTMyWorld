...
* **TXTMyWorld 전용 PRD는 아직 없음.** SVIL Outline 위키에 TXTSpace·TXTDiary PRD와 패밀리 마스터는 있으나, TXTMyWorld PRD는 미작성. 현재는 패밀리 마스터의 역할 정의만 존재.
* 열린 결정: consolidate 엔진 모델(로컬 우선), `ai_id` 자동 태깅 방식, **TXTMyWorld 이름 확정**.
* 다음 단계 후보: (1) TXTMyWorld PRD 초안 작성(마스터 §2.1·§3 기반), (2) 이름 확정, (3) 상위 3축·TXTSpace 규격 준비 상태 점검.
