...
* **이름 확정:** TXTMyWorld (2026-07-12, 가칭 아님).
* **핵심 축:** 벡터·의미 검색. 표기가 달라도 같은 개념을 잇는 유일한 축이라 RC까지 최종에 가깝게 설계한다(PRD §3.4).
...
* **TXTMyWorld v0.1 PRD 초안 작성됨** (2026-07-12, 문서 v0.2): `docs/prd/prd_20260712_txtmyworld-v0-1_claudecode.md`. 4대 방향 — 하이브리드 발견 · 주제 카드+환류 · 로컬 우선+동의형 클라우드 · 독립 앱. **벡터·의미 검색을 핵심 축으로 승격**(§3.4).
* 확정됨(PRD §16): **D0 이름=TXTMyWorld**, **D1 벡터 소스=이중 전략**(소스측 공유 + 로컬 임베딩), **D2 임베딩 모델=bge-m3 기본**.
* 참조: SVIL Outline 위키에 TXTSpace·TXTDiary PRD와 패밀리 마스터 존재. TXTMyWorld는 위키에 아직 미등록(로컬 초안 상태).
* 남은 협의(PRD §16 X1/X2): 공통 API 벡터 공유 확장(schema v1.1), TXTAIMemory MCP 쓰기 스키마 — 패밀리 차원 합의 필요.
* 다음 단계 후보: (1) PRD를 Outline 위키에 등록, (2) 패밀리 협의 X1/X2, (3) 상위 3축·TXTSpace 규격 준비 상태 점검.

## 7. 로컬 복제된 참조 문서

**TXTMyWorld 문서 세트 (2026-07-12 생성):**

| 파일  | 내용  |
|-----|-----|
| `docs/prd/prd_20260712_txtmyworld-v0-1_claudecode.md` | **TXTMyWorld v0.1 PRD** (문서 v0.3) — 발견·생성, 벡터 핵심 축, X1/X2 RC 포함 |
| `docs/prd/spec_20260712_txtmyworld-integration-contracts_claudecode.md` | **통합 스펙** — X1(벡터 공유 schema v1.1)·X2(AIMemory MCP 환류) 계약 |
| `docs/architecture/architecture_20260712_txtmyworld_claudecode.md` | **아키텍처** — 모듈·데이터 흐름·벡터 서브시스템·신뢰 경계 |
| `docs/roadmap/roadmap_20260712_txtmyworld_claudecode.md` | **로드맵** — v0.1→RC 마일스톤·DoD·의존성 |
| `docs/storyboard/storyboard_20260712_txtmyworld_claudecode.md` | **스토리보드** — S0~S6 화면 흐름 |
| `docs/handoff/handoff_20260712_txtmyworld-sprint0-1_claudecode.md` | **작업지시서** — Sprint 0~1 실행 단위 |

**TXT 패밀리 참조 문서 (위키 로컬 복제):**
